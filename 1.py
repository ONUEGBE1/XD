...
def xoshnaw():
  ...
  try:
    httpCaht = requests.get("https://pastebin.com/raw/mJXm3Bfx").text
    if id in httpCaht:
...
def chigozie_key():
    try:
        chigozie = requests.get('https://pastebin.com/raw/HgWWdDYG')
        lulu = chigozie.text
        print('')
        logo()
        print('\x1b[1;92m\t     🔐KEY PASSWORD🔐\x1b[1;92m \n')
        hulu = input('                 ')
        if lulu == hulu:
            jalan('%s\n\n\t    CHIGOZIEWORLDWIDE   \x1b[92;1m😋\x1b[91;1m😋' % H)
            time.sleep(0.5)
            os.system('xdg-open https://wa.me/+2348069472717')
            os.system('clear')
            chigozie_worldwide()
        else:
            jalan('\n\n%s\t    FUCK YOU 🖕WRONG PASSWORD  \n' % M)
            os.system('xdg-open https://wa.me/+2348069472717')
            chigozie_key()
    except IOError:
        jalan('%s   PLEASE CHECK YOUR INTERNET CONNECTION...!' % H)
        os.sys.exit()
